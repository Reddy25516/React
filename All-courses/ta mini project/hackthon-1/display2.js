...
window.onload= async function(){
    ...
    function updateBookingDate(data3){
        localStorage.setItem("booking_data", JSON.stringify(data3));
    }
...
// **************************************************************************
    ...
    function overall_sorting(){
        let data_array= getLocalStorage();

        for(let i=0;i<data_array.length;i++){
            let k= getDuration(data_array[i]);
            data_array[i].duration=k;
        }


        data_array=get_planes_by_name(data_array);
        data_array=get_planes_by_day(data_array);
...
// *************************************************************************
    ...
    let  pass_count=parseInt(JSON.parse(localStorage.getItem("passenger_count")));
    ...
    function show_cards(data){
        ...
        for(let i=0;i<data.length;i++){
            ...
            let k_stops=data[i].stops_list.length;
            ...
            <span class="return_type">${return_value}</span>
                ...
                <div class="card_details">
                    ...
                    <div id="flight-price" class="flight_price">
                        $${data[i].price[price_index]*pass_count}
                    </div>
                ...
                <div id="details-div"  class="flight_details flight_details_div" > 
                    ...
                    <div>Days available:${days_available_array}</div>
            ...
            {
            one_card= document.createElement('div');
            one_card.classList.add("one_card")
            cards_body.appendChild(one_card);
            stops_city=["mumbai","uk","india"]

            for(let j=0;j<=k_stops;j++){
                card= document.createElement("div");
                    card.innerHTML=
            `
            <div class="card">
            ...
            <span class="return_type">${return_value}</span>
                ...
                <div class="card_details">
                    <div class="card_div">
                        <div>${stops_city[j]}, ${data[i].source_country}</div>
                        <div>${data[i].Arrival_time[0]}h:${data[i].Arrival_time[1]}m</div>
    ...
    //  flight details display operation 
